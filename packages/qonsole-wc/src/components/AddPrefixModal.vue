  ...
  <dialog ref="dialogRef" class="prefix-dialog" @close="emit('close')">
    <form method="dialog" @submit.prevent>
      ...
      <div class="field-row">
        ...
        />
        <button type="button" @click="onLookup" :disabled="!inputPrefix || lookupLoading">
      ...
      <div class="field-row">
        ...
        <input id="qc-input-uri" v-model="inputUri" type="text" placeholder="https://…" />
      </div>
      ...
      <div class="dialog-buttons">
        ...
        <button type="button" class="btn-primary" @click="onAdd" :disabled="!inputPrefix">Add prefix</button>
      </div>
    </form>
  </dialog>
</template>
...
function onAdd() {
  if (!inputPrefix.value) return
  emit('add-prefix', { prefix: inputPrefix.value.trim(), uri: inputUri.value.trim() })
  emit('close')
}
...
.prefix-dialog::backdrop { background: rgba(0,0,0,0.4); }
.prefix-dialog h4 { margin: 0 0 1rem; }
.field-row {
  display: flex;
...
.field-row input { padding: 0.3rem 0.5rem; border: 1px solid #ccc; border-radius: 3px; }
.dialog-buttons { display: flex; justify-content: flex-end; gap: 0.5rem; margin-top: 1rem; }
.btn-primary { background: #337ab7; color: #fff; border: none; border-radius: 3px; padding: 0.3rem 0.75rem; cursor: pointer; }
.btn-primary:hover { background: #286090; }
.btn-primary:disabled { opacity: 0.6; cursor: default; }
.lookup-error { color: #c00; font-size: 0.875rem; margin-top: 0.25rem; }
</style>
