...
(function (mod) {
  ...
  ContentEditableInput.prototype = copyObj({
    init: function (display) {
      ...
      on(div, 'input', function () {
        ...
        if (cm.isReadOnly() || !input.pollContent()) { runInOp(input.cm, function () { regChange(cm) }) }
      })
  ...
  function buildLineContent (cm, lineView) {
    ...
    var builder = {pre: elt('pre', [content], 'CodeMirror-line'),
      content: content,
      col: 0,
      pos: 0,
      cm: cm,
  ...
  function insertLineContent (line, builder, styles) {
    ...
    for (;;) {
      if (nextChange == pos) { // Update current marker set
        ...
        if (endStyles) {
          for (var j = 0; j < endStyles.length; j += 2) { if (endStyles[j + 1] == nextChange) spanEndStyle += ' ' + endStyles[j] }
        }
  ...
  var off = CodeMirror.off = function (emitter, type, f) {
    if (emitter.removeEventListener) { emitter.removeEventListener(type, f, false) } else if (emitter.detachEvent) { emitter.detachEvent('on' + type, f) } else {
      ...
      for (var i = 0; i < handlers.length; ++i) { if (handlers[i] == f) { handlers.splice(i, 1); break } }
    }
  }
